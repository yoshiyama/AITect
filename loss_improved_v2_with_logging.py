...
class ImprovedDetectionLossWithLogging(nn.Module):
    """ログ機能付きの改善された検出損失"""
    ...
    def forward(self, predictions, targets):
        ...
        for b in range(batch_size):
            ...
            if gt_boxes.numel() == 0:
                ...
                num_neg += pred.shape[0]
                total_anchors += pred.shape[0]
                continue
            ...
            # 1. For each GT, find best matching prediction
            ...
            num_neg_batch = neg_mask.sum().item()
            num_ignore_batch = ignore_mask.sum().item()
            
            num_pos += num_pos_batch
            num_neg += num_neg_batch
            num_ignore += num_ignore_batch
            total_anchors += pred.shape[0]
            
            # Calculate average IoU for positive samples
            if num_pos_batch > 0:
                pos_ious = best_gt_per_pred[pos_mask]
                sum_pos_iou += pos_ious.sum().item()
            
            # Classification loss
            obj_targets = torch.zeros_like(pred_obj)
            obj_targets[pos_mask] = 1.0
            
            # Only compute loss for pos and neg samples
        ...
        self.last_stats = {
            'cls_loss': total_cls_loss.item() if torch.is_tensor(total_cls_loss) else total_cls_loss,
            'reg_loss': total_reg_loss.item() if torch.is_tensor(total_reg_loss) else total_reg_loss,
            'total_loss': total_loss.item() if torch.is_tensor(total_loss) else total_loss,
            'num_pos': num_pos,
            'num_neg': num_neg,
            'num_ignore': num_ignore,
            'positive_ratio': num_pos / max(total_anchors, 1),
            'negative_ratio': num_neg / max(total_anchors, 1),
            'ignore_ratio': num_ignore / max(total_anchors, 1),
            'avg_iou_positive': sum_pos_iou / max(num_pos, 1)
        }
